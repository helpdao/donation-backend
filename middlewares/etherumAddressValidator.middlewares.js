...
module.exports.isAddress = (address) => {
	return new Promise(async (resolve, reject) => {
		...
		console.log("Lower:" + isHexLower.test(address) +" Length: " + address.length +  " UPPER: " + isHexUpper.test(address))
	    if (!isHex.test(address)) {
		 ...
		 console.log("RESULT: " + /^(0x)?[0-9a-f]{40}$/i.test(address))
	        ...
	        isChecksumAddress(address).then(data => resolve(data)).catch(err => reject(err));
	    }
	});
};
